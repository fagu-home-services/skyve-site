...
import { LI_HERO } from "@/lib/licenses-data";

export function LIHero() {
  return (
      ...
      <div className="container-skyve grid items-stretch gap-8 lg:grid-cols-2">
        ...
        <div className="flex flex-col justify-center">
          <p className="eyebrow text-ridge">
            {LI_HERO.level} <span className="text-ink-50">•</span> {LI_HERO.section}
          </p>
          <h1 className="mt-3 font-serif text-4xl font-bold leading-tight text-horizon sm:text-5xl">
            {LI_HERO.title}
          </h1>
          <p className="mt-4 max-w-md text-sm leading-relaxed text-ink-70">{LI_HERO.desc}</p>

          <div className="mt-8 grid grid-cols-3 gap-5">
            {LI_HERO.features.map((f) => (
              <div key={f.title}>
        ...
        <div className="relative overflow-hidden rounded-2xl">
          <Photo
            src={LI_HERO.image}
            seed="licenses-hero"
            alt="Skyve-protected Washington home at dusk"
            priority
          ...
          <div className="absolute inset-x-4 bottom-4 rounded-xl bg-horizon-deep/92 p-6 backdrop-blur-sm sm:inset-x-6 sm:bottom-6">
            <p className="text-base font-bold text-clear">{LI_HERO.overlayTitle}</p>
            <p className="mt-2 max-w-md text-xs leading-relaxed text-mist-soft/80">{LI_HERO.overlayDesc}</p>
            <div className="mt-5 grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-4">
              {LI_HERO.overlayStats.map((s) => (
                <div key={s.label}>
