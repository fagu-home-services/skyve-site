...
export function CalcHero({ data }: { data: CalcData }) {
  return (
    <>
        ...
        <div className="container-skyve grid gap-8 lg:grid-cols-[1fr_1.05fr] lg:items-center">
          ...
          <div>
            ...
            <div className="mt-5 flex items-center gap-2 text-sm">
              ...
              <span className="text-ink-50">{data.rating}</span>
            </div>
          </div>

          {/* Right */}
          <div className="relative">
            <Photo src={data.image} alt={data.titleLines.join(" ")} className="aspect-[16/10] rounded-2xl shadow-elevated" />
            <div className="absolute bottom-4 left-4 flex max-w-xs items-start gap-3 rounded-xl bg-horizon-deep/90 p-4 text-clear shadow-elevated backdrop-blur-sm">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-accent/20 ring-1 ring-accent/50">
                <ShieldCheck className="h-4 w-4 text-accent" />
              </span>
              <div>
                <p className="text-sm font-bold">{data.badge.title}</p>
                <p className="mt-0.5 text-[11px] leading-snug text-mist-soft/80">{data.badge.desc}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="border-y border-mist bg-clear py-5">
        <div className={`container-skyve grid gap-6 sm:grid-cols-2 ${data.steps.length === 4 ? "lg:grid-cols-4" : "lg:grid-cols-3"}`}>
          {data.steps.map((s) => (
            <div key={s.n} className="flex items-center gap-3">
              <span
                className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold ${
                  s.n === 1 ? "bg-ridge text-clear" : "bg-mist text-ink-50"
                }`}
              >
                {s.n}
              </span>
