...
import { ArrowRight, Clock, ShieldCheck, CheckCircle2, Award } from "lucide-react";
import { Breadcrumb } from "@/components/shared/Breadcrumb";
import { COMPANY } from "@/lib/company";
import type { MaterialPage } from "@/lib/material-pages";
import { resolveImg } from "@/components/shared/Photo";

export function MaterialHero({ data }: { data: MaterialPage }) {
  return (
    <section className="relative overflow-hidden bg-horizon-deep">
      ...
      <div className="absolute right-6 top-1/2 hidden w-64 -translate-y-1/2 rounded-2xl bg-horizon-deep/85 p-6 text-clear shadow-elevated backdrop-blur-sm lg:block">
        <p className="font-serif text-lg font-bold">{data.whyCard.title}</p>
        <ul className="mt-3 space-y-2">
          {data.whyCard.items.map((item) => (
      ...
      <div className="container-skyve relative pt-24 pb-12 lg:pt-28">
        <div className="pb-5">
          <Breadcrumb
            ...
            items={[
              { label: "Home", href: "/" },
              data.breadcrumbParent ?? { label: "Roofing Services", href: "/services" },
              { label: data.eyebrow },
            ]}
          />
        </div>
        ...
        <div className="max-w-xl text-clear">
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            ...
            >
              View Shingle Options <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
